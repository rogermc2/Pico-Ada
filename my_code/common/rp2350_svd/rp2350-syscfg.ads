...
package RP2350.SYSCFG is
   ...
   subtype PROC_CONFIG_PROC0_HALTED_Field is RP2350.Bit;
   subtype PROC_CONFIG_PROC1_HALTED_Field is RP2350.Bit;

   --  Configuration for processors
   type PROC_CONFIG_Register is record
      --  Read-only. Indication that proc0 has halted
      PROC0_HALTED  : PROC_CONFIG_PROC0_HALTED_Field;
      --  Read-only. Indication that proc1 has halted
      PROC1_HALTED  : PROC_CONFIG_PROC1_HALTED_Field;
      --  unspecified
   ...
   subtype PROC_IN_SYNC_BYPASS_HI_GPIO_Field is RP2350.UInt16;
   subtype PROC_IN_SYNC_BYPASS_HI_USB_DP_Field is RP2350.Bit;
   subtype PROC_IN_SYNC_BYPASS_HI_USB_DM_Field is RP2350.Bit;
   subtype PROC_IN_SYNC_BYPASS_HI_QSPI_SCK_Field is RP2350.Bit;
   subtype PROC_IN_SYNC_BYPASS_HI_QSPI_CSN_Field is RP2350.Bit;
   subtype PROC_IN_SYNC_BYPASS_HI_QSPI_SD_Field is RP2350.UInt4;
   ...
   type PROC_IN_SYNC_BYPASS_HI_Register is record
      ...
      USB_DP         : PROC_IN_SYNC_BYPASS_HI_USB_DP_Field := 16#0#;
      USB_DM         : PROC_IN_SYNC_BYPASS_HI_USB_DM_Field := 16#0#;
      QSPI_SCK       : PROC_IN_SYNC_BYPASS_HI_QSPI_SCK_Field := 16#0#;
      QSPI_CSN       : PROC_IN_SYNC_BYPASS_HI_QSPI_CSN_Field := 16#0#;
   ...
   subtype DBGFORCE_SWDO_Field is RP2350.Bit;
   subtype DBGFORCE_SWDI_Field is RP2350.Bit;
   subtype DBGFORCE_SWCLK_Field is RP2350.Bit;
   subtype DBGFORCE_ATTACH_Field is RP2350.Bit;

   --  Directly control the chip SWD debug port
   type DBGFORCE_Register is record
      --  Read-only. Observe the value of SWDIO output.
      SWDO          : DBGFORCE_SWDO_Field := 16#0#;
      --  Directly drive SWDIO input, if ATTACH is set
      SWDI          : DBGFORCE_SWDI_Field := 16#1#;
      --  Directly drive SWCLK, if ATTACH is set
      SWCLK         : DBGFORCE_SWCLK_Field := 16#1#;
      --  Attach chip debug port to syscfg controls, and disconnect it from
      --  external SWD pads.
      ATTACH        : DBGFORCE_ATTACH_Field := 16#0#;
      --  unspecified
   ...
   subtype MEMPOWERDOWN_SRAM_Element is RP2350.Bit;

   --  MEMPOWERDOWN_SRAM array
   type MEMPOWERDOWN_SRAM_Field_Array is array (0 .. 9)
   ...
   subtype MEMPOWERDOWN_USB_Field is RP2350.Bit;
   subtype MEMPOWERDOWN_ROM_Field is RP2350.Bit;
   subtype MEMPOWERDOWN_BOOTRAM_Field is RP2350.Bit;
   ...
   type MEMPOWERDOWN_Register is record
      ...
      BOOTRAM        : MEMPOWERDOWN_BOOTRAM_Field := 16#0#;
      --  unspecified
   ...
   subtype AUXCTRL_AUXCTRL_Field is RP2350.Byte;
   ...
   type SYSCFG_Peripheral is record
      ...
      PROC_IN_SYNC_BYPASS    : aliased RP2350.UInt32;
      --  For each bit, if 1, bypass the input synchronizer between that GPIO
      --  and the GPIO input register in the SIO. The input synchronizers
      --  should generally be unbypassed, to avoid injecting metastabilities
      --  into processors. If you're feeling brave, you can bypass to save two
